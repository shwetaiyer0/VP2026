...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const videoUrl =
    "https://www.youtube.com/embed/aBADWpeitEI?controls=1&autoplay=1";
...
function initMap() {
  ...
  fetch("js/classData.json")
    ...
    .then(data => {
      ...
      // Read area/day from the URL and apply filters
      loadFiltersFromUrl();
    })
    .catch(error => {
...
function populateDropdowns() {
  ...
  populateSelect(
    "areaDropdown",
    ["AREA", "ALL", ...Array.from(areaSet).sort()]
  );
  ...
  const availableDays = Array.from(daySet).filter(day =>
  ...
  populateSelect(
    "dayDropdown",
    ["DAY OF THE WEEK", "ALL", ...sortedDays]
  );
}
...
function filterClasses(updateUrl = true) {
  ...
  if (
    ...
    (area === "ALL" || isAreaDefault) &&
    day !== "ALL" &&
    !isDayDefault
  ) {
    filtered = classData.filter(cls =>
      cls.day === day
    );
  }
    ...
    <div class="results-grid">
      ...
      ${filtered.map((cls, idx) => `
        ...
        >
          ...
          ${
            cls.youth
              ? `<h2 class="youth-badge">Youth Class</h2>`
              : ""
          }
...
// /weekly?area=Baner
// /weekly?day=SATURDAY
// /weekly?area=Baner&day=SATURDAY
// ============================================================
...
function loadFiltersFromUrl() {
  ...
  if (
    area &&
    Array.from(areaDropdown.options).some(
      option => option.value === area
    )
  ) {
    areaDropdown.value = area;
  }

  if (
    day &&
    Array.from(dayDropdown.options).some(
